...
// ================================================================
  ...
  public function editsoal($id)
  {
      ...
      $countSoal = Detailsoal::where('id_soal', '=', $id)->count();
      return view('guru.editsoal', compact('user', 'school', 'id_soal', 'soal', 'countSoal'));
    }else{
  ...
  //@meztr Sunday,9Des18
  public function duplicateformsoal()
  {
    if (Request::ajax()) {
      if (Input::get('paket') != "") {
        if (Input::get('deskripsi') != "") {
          if (Input::get('kkm') != "") {
            if (Input::get('skor') != "") {
              if (Input::get('waktu') != "") {              
                $originsoal = Soal::where('id', '=', Input::get('id_soal'))->first();
                $simpan = $originsoal->replicate();
                $simpan->paket = Input::get('paket');
                $simpan->deskripsi = Input::get('deskripsi');
                $simpan->kkm = Input::get('kkm');
                $simpan->waktu = Input::get('waktu');
                ...
                $butir_array = json_decode($_REQUEST["butir_soals"], true);              
                foreach ($butir_array as $soal) {
                  $newDetailSoals = Detailsoal::where('id', '=', $soal)->first();
                  $newSoal = $newDetailSoals->replicate();
                  $newSoal->id_soal = $simpan->id;
                  $newSoal->score = Input::get('skor');
                  $newSoal->save();
                }
                return 'berhasil';
              } else {
              ...
              return "<b>Error:</b> Anda belum menuliskan skor soal";
            }
          } else {
            return "<b>Error:</b> Anda belum menuliskan kkm soal";
          }
  ...
  {
    Detailsoal::where('id_soal', '=', $id)->delete(); 
